...
class LLMService:
    ...
    async def get_next_event(self, level: int = 1) -> dict:
        ...
        prompt = f"""请生成一个职场挑战场景，难度级别为Level {level}，挑战类型是：{selected_type}。

【多样化要求】
1. 每次生成的问题类型必须不同，本次选择：{selected_type}
2. 标题要独特，不要使用"职场挑战"这种通用词，使用具体场景命名
3. 描述要有画面感，包含具体的人物、场景、冲突
4. 场景必须是真实的职场挑战，符合当前级别难度
5. 【重要】禁止使用之前可能生成过的类似表达

【返回格式】
返回纯净JSON格式，不要用markdown代码块包裹，不要有任何多余文字。JSON包含以下字段：
   - level: 当前级别数字（整数）
   - title: 关卡标题（如 "Level {level} - 具体场景名"），禁止使用"xxx挑战"这种占位符
   - description: 场景描述（50-100字，包含具体场景、人物、冲突，要有画面感）
   - difficulty: 难度系数（1-7）
