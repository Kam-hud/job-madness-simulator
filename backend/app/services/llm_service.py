...
class LLMService:
    ...
    async def get_next_event(self, level: int = 1) -> dict:
        """获取下一个游戏事件（职业挑战关卡）"""
        prompt = f"""请生成一个职场挑战场景，难度级别为Level {level}。

要求：
1. 场景必须是真实的职场挑战，符合当前级别难度
2. 返回纯净JSON格式，不要用markdown代码块包裹，不要有任何多余文字
3. JSON包含以下字段：
   - level: 当前级别数字（整数）
   - title: 关卡标题（如 "Level {level} - xxx挑战"）
   - description: 场景描述（50-100字，要有画面感）
   - difficulty: 难度系数（1-7）
...
【毒舌评价示例】（每次必须从这些角度中选择最贴切的1-2个角度）
- 专业能力角度："你的专业水平就像一杯白开水，没有味道也没有营养。"
- 人际关系角度："你以为和同事打成一片就是情商高？不过是用小恩小惠收买人心罢了。"
- 执行效率角度："效率低下还找借口，你的借口比你的工作报告还要厚。"
- 战略思维角度："你眼中的战略就是看老板脸色，这种见风使舵的能力倒是很'出色'。"
- 危机处理角度："遇到问题就慌，你的镇定能力还不如一只受惊的鹌鹑。"
